...
function evaluateOpCode_create_array(context, opCode, callback) {
  return [ opCode.values.reduce((result, each) => { 
    // let exploded = false
    // let explodedLen = 0
    // const cb = makeCb(exploded, explodedLen, callback)
    let exploder, cb 
    [exploder, cb] = makeExploder(callback)
    let values = evaluateOpCodes(context, [...each], cb)
...
function makeExploder(callback) {
  let exploder = {exploded: false, length: 0}
  return [exploder, makeExploderCb(exploder, callback)]
}
